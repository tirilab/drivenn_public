...
def get_tvt(direct=""):
    with open(direct + "data/model_data/TTS/test_pairs.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        test_pairs = list(csv_reader)
    with open(direct + "data/model_data/TTS/test_y.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        test_y = list(csv_reader)
        test_Y = [[int(v) for v in se] for se in test_y] 
    with open(direct + "data/model_data/TTS/val_pairs.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        val_pairs = list(csv_reader)
    with open(direct + "data/model_data/TTS/val_y.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        val_y = list(csv_reader)
        val_Y = [[int(v) for v in se] for se in val_y]
    with open(direct + "data/model_data/TTS/train_pairs.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        train_pairs = list(csv_reader)
    with open(direct + "data/model_data/TTS/train_y.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
...
def get_cvd_tvt(direct=""):
    with open(direct + "data/model_data/TTS/cvd/test_pairs.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        test_pairs = list(csv_reader)
    with open(direct + "data/model_data/TTS/cvd/test_y.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        test_y = list(csv_reader)
        test_Y = [[int(v) for v in se] for se in test_y] 
    with open(direct + "data/model_data/TTS/cvd/val_pairs.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        val_pairs = list(csv_reader)
    with open(direct + "data/model_data/TTS/cvd/val_y.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        val_y = list(csv_reader)
        val_Y = [[int(v) for v in se] for se in val_y]
    with open(direct + "data/model_data/TTS/cvd/train_pairs.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        train_pairs = list(csv_reader)
    with open(direct + "data/model_data/TTS/cvd/train_y.csv", "r") as read_obj:
        csv_reader = csv.reader(read_obj)
        train_y = list(csv_reader)
        train_Y = [[int(v) for v in se] for se in train_y]
    return train_pairs, train_Y, val_pairs, val_Y, test_pairs, test_Y
